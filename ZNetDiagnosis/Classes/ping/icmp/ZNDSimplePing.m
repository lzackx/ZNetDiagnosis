...
- (void)didFailWithError:(NSError *)error {
    id<ZNDSimplePingDelegate>  strongDelegate;
...
- (NSData *)pingPacketWithType:(uint8_t)type payload:(NSData *)payload requiresChecksum:(BOOL)requiresChecksum {
    ...
    if (requiresChecksum) {
        ...
        icmpPtr->checksum = in_checksum(packet.bytes, packet.length);
    }
...
- (void)sendPingWithData:(NSData *)data {
    ...
    if (payload == nil) {
        ...
        // Our dummy payload is sized so that the resulting ICMP packet, including the ICMPPacketHeader, is
...
+ (NSUInteger)icmpHeaderOffsetInIPv4Packet:(NSData *)packet {
    ...
    NSUInteger                  result;
    const struct IPv4PacketHeader *   ipPtr;
    size_t                      ipHeaderLength;
    
    result = NSNotFound;
    if (packet.length >= (sizeof(IPv4PacketHeader) + sizeof(ICMPPacketHeader))) {
        ipPtr = (const IPv4PacketHeader *) packet.bytes;
        if ( ((ipPtr->versionAndHeaderLength & 0xF0) == 0x40) &&            // IPv4
            ( ipPtr->protocol == IPPROTO_ICMP ) ) {
            ipHeaderLength = (ipPtr->versionAndHeaderLength & 0x0F) * sizeof(uint32_t);
            if (packet.length >= (ipHeaderLength + sizeof(ICMPPacketHeader))) {
 ...
 *
 *      * It needs to zero out the `checksum` field of the ICMPPacketHeader in order to do
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (icmpHeaderOffset != NSNotFound) {
        icmpPtr = (struct ICMPPacketHeader *) (((uint8_t *) packet.mutableBytes) + icmpHeaderOffset);
        
        receivedChecksum   = icmpPtr->checksum;
        icmpPtr->checksum  = 0;
        calculatedChecksum = in_checksum(icmpPtr, packet.length - icmpHeaderOffset);
...
- (BOOL)validatePing6ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    BOOL                    result;
    const ICMPPacketHeader *      icmpPtr;
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge ZNDSimplePing *) info;
    assert([obj isKindOfClass:[ZNDSimplePing class]]);
 ...
 */
+(const struct ICMPPacketHeader *)icmpInPacket:(NSData *)packet
{
    const struct ICMPPacketHeader *result;
    result = nil;
